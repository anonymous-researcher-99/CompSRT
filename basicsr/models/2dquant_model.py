                ...
                for k, t in state_dict.items():
                    ...
                    if wscope is not None:
                        if wscope in quant_scopes:
                            nbytes = (4/8) * t.nelement() #*0.6 + 0.125 * t.nelement()  #two bits + 40% pruning ratio + 1 bit mask # for 2dquants calculation, can ignore alpha and beta params in the calc must add this
        ...
        def analyze_save_dict_2dquant(save_dict: dict, topk: int = 10) -> None:
            ...
            for model_name, state_dict in save_dict.items():
                ...
                for k, t in state_dict.items():
                    ...
                    if wscope is not None:
                        if wscope in quant_scopes:
                            nbytes = (4/8) * t.nelement() * 1  #4 bits + 0% pruning ratio + 1 bit mask # for 2dquants calculation, can ignore alpha and beta params in the calc must add this
        ...
        while retry > 0:
            ...
            try:
                ...
                if printed==0: 
                    ...
                    print(f"{size_mb:.2f} 4bit MiB ours+ 40% pruning",flush=True)

                    size_mb = analyze_save_dict_noprune(save_dict)
                    print(f"{size_mb:.2f} 4bit MiB ours+ no pruning",flush=True)

                    size_mb = analyze_save_dict_2dquant(save_dict)
                    print(f"{size_mb:.2f} 4bit MiB 2dquant",flush=True)
