...
class Selective_Scan(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor, prompt):
        ...
        if hasattr(self, "x_proj_weight_quant"):
            w = self.x_proj_weight_quant()
        else:
            w = self.x_proj_weight

        x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs.view(B, K, -1, L), w)
        dts, Bs, Cs = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=2)
        if hasattr(self, "dt_projs_weight_quant"):
            w_dt = self.dt_projs_weight_quant()
        else:
            w_dt = self.dt_projs_weight
        dts = torch.einsum("b k r l, k d r -> b k d l", dts.view(B, K, -1, L), w_dt)
